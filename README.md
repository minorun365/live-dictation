...
セッションごとに録音音声、Markdownの文字起こし、直近5分の要約、時刻付きイベント、録音中の画面を保存します。日本語モードの録音は、自分の声を `audio-self.caf`、相手の声を `audio-others.caf` として別々に保存するため、あとから片方だけを取り出して読み直せます（英語モードは従来どおり `audio.caf` の1本です）。録音開始と同時にメインディスプレイ全体を自動取得し、1秒ごとのJPEG画像として `screenshots/` に記録します。画面取得に失敗した場合も、音声録音と文字起こしは継続します。
