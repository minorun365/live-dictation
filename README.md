...
音声入力アプリやmacOSの音声認識は無視するので、普段のディクテーションで録音が始まることはありません。「録音を開始」を押して自分で始めた録音は、会議アプリの状態にかかわらず自動では停止しません。この自動検知はアプリが起動している間だけ働くため、常に効かせたい場合はシステム設定の「一般」→「ログイン項目と機能拡張」へ文字起こしちゃんを追加してください。
