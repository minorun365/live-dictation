...
上部でモードを選び、「録音を開始」を押します。日本語モードは日本語文字起こしと要約、英語モードは英語文字起こし・日本語訳・要約を表示します。

対面モードは、Web会議ではなく同じ部屋で交わす会話のためのものです。全員の声が1本のマイクへ入るため発言者を見分けられないので、文字起こしに「自分：」「相手：」は付きません。会議室ではMacの画面に残す価値がないため、画面の記録も行いません。左の履歴には、会議全体から生成した短い日本語タイトルが新しい順に並びます。履歴を選ぶと、保存済みの全文と要約を読み返せます。

会議アプリ（Zoom、Microsoft Teams、Webex、Slackのハドル、Discord）がマイクを使い始めると、自動で録音を開始します。その会議アプリがマイクを離してから1分たつと自動で停止します。ブラウザーで行うGoogle Meetなどは、マイクと再生の両方を使っているときだけ会議とみなします。

音声入力アプリやmacOSの音声認識は無視するので、普段のディクテーションで録音が始まることはありません。「録音を開始」を押して自分で始めた録音は、会議アプリの状態にかかわらず自動では停止しません。

ウィンドウを閉じてもアプリはメニューバーに残り、会議の検知を続けます。録音中もウィンドウを閉じられます。メニューバーのアイコンから、いまの状態の確認、録音の開始と停止、ウィンドウの再表示ができます。終了するときは⌘Qか、メニューバーの「文字起こしちゃんを終了」を選びます。Macを再起動しても検知を続けたい場合は、システム設定の「一般」→「ログイン項目と機能拡張」へ文字起こしちゃんを追加してください。
