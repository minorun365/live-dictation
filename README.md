...
ウィンドウを閉じてもアプリはメニューバーに残り、会議の検知を続けます。アイコンから状態の確認、録音の開始と停止、ウィンドウの再表示ができます。「対面会議を開始」を選ぶと、ウィンドウを開かずに対面モードで録音を始められます（このときウィンドウで選んでいるモードは変わらないので、次のWeb会議は日本語モードのまま自動で始まります）。終了は⌘Qです。
